...
const getEmailStatusText = (verification: any) => {
  if (!verification.formatValid) {
    return "Invalid format";
  }
  if (!verification.domainExists) {
    return "Domain doesn't exist";
  }
  if (!verification.hasValidMxRecords) {
    return "No mail server";
  }
  if (verification.smtpCheck) {
    return "Valid email";
  }
  return "Mail server exists";
};
                            ...
                            <div>
                              <ul className="space-y-3">
                                ...
                                <li className="flex justify-between items-center">
                                  ...
                                  <span className={`font-mono ${sslData.daysRemaining > 30 ? 'text-green-400' : sslData.daysRemaining > 14 ? 'text-yellow-400' : 'text-red-400'}`}>
                        ...
                        <div className="p-4 bg-gray-800/50 rounded-md border border-green-500/30">
                          <p className="text-gray-400 text-sm flex items-start">
                            ...
                            <span>
                              ...
                              and will expire in 
                              <span className={`font-semibold mx-1 ${sslData.daysRemaining > 30 ? 'text-green-400' : sslData.daysRemaining > 14 ? 'text-yellow-400' : 'text-red-400'}`}>
                                {sslData.daysRemaining != null ? `${sslData.daysRemaining} days` : 'Unknown'}
                              </span>. 
                  ...
                  {emailData && emailData.mailServers && emailData.mailServers.length > 0 && (
                    <div className="mt-6">
                      ...
                      <div className="bg-gray-800 p-4 rounded-md">
                        <div className="terminal-text mb-2">$ dig {domain} MX</div>
                        <div className="border-t border-green-500/30 pt-2 mt-2">
                          {emailData.mailServers.map((server: any, index: number) => (
                            <div key={index} className="font-mono text-sm text-gray-300 mb-1">
